...
def update_csproj_version(csproj_path: Path, new_version: str) -> bool:
    ...
    patterns = [
        ...
        (
            ...
            f"<AssemblyVersion>{version_4}</AssemblyVersion>",
        ),
        (
            r"<FileVersion>[\d.]+</FileVersion>",
            f"<FileVersion>{version_4}</FileVersion>",
        ),
    ]
...
示例:
  ...
  python bump.py --version 1.2.3.4
    ...
    parser.add_argument(
        "-v", "--version", required=True, help="新版本号 (格式: 1.0.0 或 1.0.0.0)"
    )

    args = parser.parse_args()

    # 查找 .csproj 文件（支持从仓库根目录或 scripts/ 目录运行）
    script_dir = Path(__file__).resolve().parent
    # 脚本在 scripts/ 下，父目录即为仓库根目录
    repo_root = script_dir.parent
    csproj_path = repo_root / "Desuwa.csproj"
